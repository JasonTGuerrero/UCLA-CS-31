...
void test()
{
    ...
    assert( BOARD.isGameOver() == false );
    BOARD.setComputerTurn();
    BOARD.setComputerTotal(4);
    BOARD.setComputerScore(6);
    BOARD.setHumanTurn();
    BOARD.setHumanScore(12);
    BOARD.setHumanTotal(14);
    BOARD.setRolledValue(7);
    assert( BOARD.getComputerTotal() == 4 );
    assert( BOARD.getHumanTotal() == 14 );
    assert( BOARD.getRolledValue() == 7);
    assert( BOARD.getComputerScore() == 6 );
    assert( BOARD.getHumanScore() == 12);
    assert( BOARD.isHumanWinner() == false );
    assert( BOARD.isHumanTurn() == true );
    assert( BOARD.isGameOver() == false );
...
int main()
{
    ...
    bool testmode = false; // boolean test mode switch for void test function to use asserts
